...
<p align="center">
  <strong>Hybrid Spatial–Temporal Intrusion Detection for Controller Area Network traffic</strong>
</p>
...
CAN bus traffic. It combines a lightweight **Spatial Transformer**, which
learns a representation of each CAN frame, with a **Temporal GRU**, which
models sequences of frames and engineered traffic features.
...
- Spatial Transformer embeddings for individual CAN frames.
- Temporal GRU modeling over sequences of 32 valid frames.
- Temporal features including inter-arrival time, jitter, Hamming distance,
  frame rate, ID rate, entropy, dominant-ID ratio, and bus-load proxy.
...
The hybrid model is trained in two stages:

1. Load the pretrained Spatial Transformer and freeze its encoder.
2. Fine-tune the complete hybrid model with a lower learning rate.
...
│   ├── model/            # Transformer, GRU, embeddings, and hybrid model
...
# 2. Train and evaluate the spatial model
PYTHONPATH=src python src/train_spatial.py
PYTHONPATH=src python src/eval_spatial.py

# 3. Train and evaluate the hybrid model
PYTHONPATH=src python src/train_hybrid.py
PYTHONPATH=src python src/eval_hybrid.py
```

The hybrid training step requires the spatial checkpoint generated by
`train_spatial.py`, so the spatial model must be trained first.
...
| Spatial training | `src/train_spatial.py` | Trains the frame-level Spatial Transformer. |
| Spatial evaluation | `src/eval_spatial.py` | Generates classification metrics and a confusion matrix. |
| Hybrid training | `src/train_hybrid.py` | Trains the Spatial Transformer + Temporal GRU model in two stages. |
| Hybrid evaluation | `src/eval_hybrid.py` | Evaluates the hybrid model and logs metrics and figures. |
...
| `checkpoints/spatial_best.keras` | Best Spatial Transformer checkpoint. |
| `checkpoints/hybrid_best.keras` | Best hybrid checkpoint selected by validation macro-F1. |
| `checkpoints/hybrid_final.keras` | Final hybrid model after fine-tuning. |
| `checkpoints/temporal_norm_stats.json` | Train-derived temporal normalization statistics. |
