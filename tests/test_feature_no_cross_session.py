"""
Pastikan fitur temporal (delta_t, jitter, dst) tidak pernah dihitung
lintas session_id. Deteksi tidak langsung: baris pertama tiap session_id
(setelah diurutkan per session_id + Timestamp) harus punya delta_t == 0,
...
def test_first_row_per_session_has_zero_delta_t(split_name):
    ...
    non_zero = first_rows.filter(pl.col("delta_t") != 0.0)

    assert len(non_zero) == 0, (
        "Ada baris pertama sesi dengan delta_t != 0 -- indikasi delta_t "
